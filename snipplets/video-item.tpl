...
<div class="{% if not thumb %}js-video {% if product_video %}js-video-product{% endif %}{% endif %} embed-responsive embed-responsive-16by9 visible-when-content-ready">
    ...
    {% else %}
        {% if product_video %}
            ...
            <a href="#modal-product-video" data-toggle="modal" data-modal-url="#modal-fullscreen-video" class="js-play-button js-fullscreen-modal-open player-container visible-xs">
                <div class="btn btn-primary {% if thumb %}btn-small{% endif %}">{% include "snipplets/svg/play.tpl" with {fa_custom_class: "fa-play play-icon " ~ svg_size ~ ""} %}</div>
            </a>
        {% endif %}
    ...
    {% endif %}
            <div class="btn btn-primary {% if thumb %}btn-small{% endif %}">
                {% include "snipplets/svg/play.tpl" with {fa_custom_class: "fa-play play-icon " ~ svg_size ~ ""} %}
            </div>
    {% if thumb %}
        </div>
    {% else %}
...
{% if not thumb %}
    ...
    <div class="js-video-iframe embed-responsive embed-responsive-16by9" style="display: none;" data-video-color="{{ settings.primary_brand_color | trim('#') }}">
    </div>
{% endif %}