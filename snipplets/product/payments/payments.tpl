                ...
                <div class="nav-tabs-container horizontal-container">
                    <ul class="nav nav-tabs m-right-double">
                        {% for method, installments in installments_info %}
                            ...
                            {% if loop.last and custom_payment is not null and current_language.country == 'AR' %}
                                <li id="method_{{ custom_payment.code }}" class="js-refresh-installment-data js-installments-gw-tab tab m-bottom-none" data-code="{{ custom_payment.code }}">
                                    <a href="#installment_{{ custom_payment.code }}" class="tab-link" data-toggle="tab">
                                        ...
                                        {% if custom_payment.discount > 0 %}
                                            <span class="label label-primary label-inline m-left-quarter font-small"><strong>{{ custom_payment.discount }}% {{'OFF' | translate }}</strong></span>
                                        {% endif %}
                                    </a>
                                </li>
                            {% endif %}
                    ...
                    <span class="btn btn-floating pull-right hidden-xs" data-dismiss="modal" aria-label="Close">
                      {% include "snipplets/svg/times.tpl" with {fa_custom_class: "fa-md"} %}
                    </span>
                </div>
            </div>
                                ...
                                <div class="full-width pull-left">
                                    ...
                                    {% if store_set_for_new_installments_view %}
                                        ...
                                        {% if payment_methods_config[method].show_full_installments %}
                                            ...
                                            {% include 'snipplets/product/payments/payments-info-banks.tpl' %}
                                        {% else %}

                                            {# Payments Gateways with cards only #}

                                            {% include 'snipplets/product/payments/payments-info.tpl' %}
                                        {% endif %}
                                    ...
                                    <div class="box-container">
                                        ...
                                        <h4 class="m-top-quarter">
                                            ...
                                            {% set price_with_discount = product.price - ((product.price * custom_payment.discount) / 100) %}
                                                <span class="js-installments-one-payment price-compare m-right-half">{{ product.price | money }}</span><strong class="js-price-with-discount h3 weight-strong text-primary" data-payment-discount="{{ custom_payment.discount }}">{{ price_with_discount | money }}</strong> 
                                            {% else %} 
