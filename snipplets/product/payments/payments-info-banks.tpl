          ...
          {% for installment, banks in gateways.cc.no_interest %}
            <div class="installments-container">
              ...
              <h4 class="text-primary m-top-none m-bottom-quarter">
                ...
                {{'de' | t}}
                <span class="js-modal-installment-price" data-installment="{{installment}}"> {{ (product.price / installment) | money }}</span>
              </h4>
...
{% if gateways.debit is not null or gateways.cash is not null or gateways.transfer is not null %}
  ...
  <div class="box-container">
    ...
    {% if gateways.debit is not null %}
      ...
      <h4 class="text-primary m-top-none m-bottom-quarter">{{ 'Débito' | translate }}</h4>
    ...
    {% if gateways.cash is not null %}
      ...
      <h4 class="text-primary m-top-none m-bottom-quarter">{{'Efectivo' | translate }}</h4>
      <div class="h6-xs m-bottom-half">
        <span>{{ 'Precio:' | translate }} </span><span class="js-installments-one-payment weight-strong text-primary"> {{ product.price | money }}</span>
      </div>
    ...
    {% if gateways.transfer is not null %}
      ...
      {# Transfer price #}
      {% if current_language.country == 'AR' %}
          <h4 class="text-primary m-top-none m-bottom-quarter">{{ 'Transferencia o déposito' | translate }}</h4>
          <div class="h6-xs m-bottom-half">
            <span>{{ 'Precio:' | translate }} </span><span class="js-installments-one-payment weight-strong text-primary"> {{ product.price | money }}</span>
          </div>
        {% endif %}
